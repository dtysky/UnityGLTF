...
//#endif
        ...
        {
            SketchfabRequest request = new SketchfabRequest(SketchfabPlugin.Urls.latestReleaseCheck);
            request.setCallback(OnVersionCheck);
            _api.registerRequest(request);
        }

        void OnVersionCheck(string response)
        {
            JSONNode result = Utils.JSONParse(response);
            if(result.AsArray != null && result[0]["tag_name"] != null)
            {
                string version = result[0]["tag_name"];
        ...
        private void OnExportProgress(UnityGLTF.GLTFSceneExporter.EXPORT_STEP step, float current, float total)
        {
            ...
            switch (step)
            {
                case UnityGLTF.GLTFSceneExporter.EXPORT_STEP.NODES:
                    element = "Node";
                    break;
                case UnityGLTF.GLTFSceneExporter.EXPORT_STEP.ANIMATIONS:
                    element = "Image";
                    break;
                case UnityGLTF.GLTFSceneExporter.EXPORT_STEP.SKINNING:
                    element = "Skin";
                    break;
                case UnityGLTF.GLTFSceneExporter.EXPORT_STEP.IMAGES:
                    element = "Image";
                    break;
            }
        ...
        private List<IMultipartFormSection> buildParameterDict()
        {
            List<IMultipartFormSection> _multiPart = new List<IMultipartFormSection>();
            _multiPart.Add(new MultipartFormDataSection("name", param_name));
            _multiPart.Add(new MultipartFormDataSection("description", param_description));
            _multiPart.Add(new MultipartFormDataSection("tags", "unity unity3D " + param_tags));
            _multiPart.Add(new MultipartFormDataSection("private", param_private ? "1" : "0"));
            _multiPart.Add(new MultipartFormDataSection("isPublished", param_autopublish ? "1" : "0"));
            if (param_private && param_password.Length > 0)
                _multiPart.Add(new MultipartFormDataSection("password", param_password));
